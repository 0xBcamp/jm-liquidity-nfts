...
describe("KimPair", () => {
  ...
  swapTestCases.forEach((swapTestCase, i) => {
    it(`getInputPrice:${i}`, async () => {
      ...
      expect(
        await pair.getAmountOut(swapAmount, await token0.getAddress())
      ).to.eq(expectedOutputAmount);
      await expect(
        pair.swap(
          0,
          expectedOutputAmount.valueOf() + BigInt(1),
          wallet.address,
          "0x"
        )
  ...
  optimisticTestCases.forEach((optimisticTestCase, i) => {
    it(`optimistic:${i}`, async () => {
      ...
      await expect(
        pair.swap(outputAmount.valueOf() + BigInt(2), 0, wallet.address, "0x")
      ).to.be.revertedWith("KimPair: K");
  ...
  it("swap:token0", async () => {
    ...
    await expect(pair.swap(0, expectedOutputAmount, wallet.address, "0x"))
      ...
      .to.emit(pair, "Sync")
      .withArgs(
        token0Amount.valueOf() + swapAmount.valueOf(),
    ...
    expect(reserves[1]).to.eq(
      token1Amount.valueOf() - BigInt(expectedOutputAmount)
    );
    ...
    expect(await token0.balanceOf(wallet.address)).to.eq(
      totalSupplyToken0.valueOf() -
        token0Amount.valueOf() -
        swapAmount.valueOf()
    );
    expect(await token1.balanceOf(wallet.address)).to.eq(
      totalSupplyToken1.valueOf() -
        token1Amount.valueOf().valueOf() +
        expectedOutputAmount
    );
  });
  ...
  it("swap:token1", async () => {
    ...
    await expect(pair.swap(expectedOutputAmount, 0, wallet.address, "0x"))
      ...
      .withArgs(
        ...
        token1Amount.valueOf() + swapAmount.valueOf()
      )
      .to.emit(pair, "Swap")
      .withArgs(
        wallet.address,
